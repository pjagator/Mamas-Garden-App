...
- **Captured photos didn't appear in Garden until hard refresh**: `useInventory()` was being called independently in 7 components (Garden, Map, Timeline, Wishlist, Settings, CaptureSheet, ManualEntry). Each call created its own isolated `useState` — so when CaptureSheet inserted a row to Supabase the save succeeded, but only its own throwaway local state updated. Garden's separate state never learned about the new row until `load()` ran again on mount. The DB was fine; the UI was reading stale in-memory state. Fixed by refactoring `useInventory.ts` to expose an `InventoryProvider` context that owns a single shared state instance, wrapping `AppShell` in the provider, and making `useInventory()` read from the context. Consumer call sites are unchanged.
